...
class CustomPriorityQueue:
    ...
    def update_priority(self, old_item, new_item):
        try:
...
class DemandForecast:
    ...
    def forecast_with_trend_and_seasonality(self):
        ...
        if self.order_queue:
            ...
            if order.product_id in self.inventory.products:
                ...
                if product.stock >= order.quantity:
                    product.update_stock(-order.quantity)
                    self.inventory.transaction_stack.push((order.product_id, -order.quantity))
                    print(f"Order {order.order_id} processed. Stock for product ID {order.product_id} updated.")
                else:
                    print(f"Order {order.order_id} cannot be processed. Not enough stock for product ID {order.product_id}.")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            supplier_id = input("Enter supplier ID: ").strip()
            print("================================")
            products = supplier_network.list_supplier_products(supplier_id)
